...
from protocol import normalize_airflow
...
class AcController(ABC):
    ...
    async def apply_settings(self, **fields) -> bool:
        ...
        fields는 AcStatus 필드명 기준. 기류 연동 필드(vane↔wind_free↔long_wind)의
        상호배타는 protocol.normalize_airflow()가 자동 적용된다.
        """
        raise NotImplementedError
...
class MockAcController(AcController):
    ...
    async def set_vane(self, vertical: bool, horizontal: bool) -> bool:
        await self._store.update(**normalize_airflow(
            dict(vane_vertical=vertical, vane_horizontal=horizontal)))
        logger.info("mock: vane vertical=%s horizontal=%s", vertical, horizontal)
        return True

    async def set_wind_free(self, on: bool) -> bool:
        await self._store.update(**normalize_airflow(dict(wind_free=on)))
        logger.info("mock: wind_free=%s", on)
        return True

    async def set_long_wind(self, on: bool) -> bool:
        await self._store.update(**normalize_airflow(dict(long_wind=on)))
    ...
    async def apply_settings(self, **fields) -> bool:
        fields = normalize_airflow(fields)
...
class RealAcController(AcController):
    ...
    async def set_vane(self, vertical: bool, horizontal: bool) -> bool:
        await self._store.set_desired(**normalize_airflow(
            dict(vane_vertical=vertical, vane_horizontal=horizontal)))
        if not await self._powered_on():
            return True
    ...
    async def set_wind_free(self, on: bool) -> bool:
        await self._store.set_desired(**normalize_airflow(dict(wind_free=on)))
        if not await self._powered_on():
            return True
    ...
    async def set_long_wind(self, on: bool) -> bool:
        await self._store.set_desired(**normalize_airflow(dict(long_wind=on)))
        if not await self._powered_on():
            return True
